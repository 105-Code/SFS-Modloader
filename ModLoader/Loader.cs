        ...
        private Dictionary<string, SFSMod> getModList()
        {
            ...
            foreach (FolderPath folder in modsFolders)
            {
                ...
                try
                {
                    ...
                    catch (ReflectionTypeLoadException reflectionError)
                    {
                         // verfiy if hace SFSMod class, but skips the strange null entries. Fix by u/pixel_gaming579
                        foreach (Type typeClass in reflectionError.Types)
                        {
                            if (typeClass.IsSubclassOf(typeof(SFSMod)))
                            {
                                mod = (Activator.CreateInstance(typeClass) as SFSMod);
